...
from .clip_model import build_model, tokenize
...
def clip_retrieval(pretrained='', device=None, fashion=False):
    if fashion==True:
        model, preprocess = load("ViT-B/32", device=device, jit=False)  # Must set jit=False for training
        if pretrained:
            model, msg = load_checkpoint(model, pretrained)
            print("missing keys:")
            print(msg['missing_keys'])  # 这里改成 msg['missing_keys']
    else:
        model, preprocess = load("ViT-B/16", device=device, jit=False)  # Must set jit=False for training
...
def load_checkpoint(model, pretrained):
    ...
    model.load_state_dict(checkpoint['CLIP'], strict=False)
    msg = {}
    msg['missing_keys'] = [k for k in model.state_dict().keys() if k not in checkpoint['CLIP'].keys()]
    return model, msg

def get_text_embeds_clip(data_loader, model, device, args):
    num_length=len(data_loader.dataset.text)
    text_embeds = torch.zeros(num_length, 512, dtype=torch.half).to(device)

    for text, index in data_loader:
        text_output = model.encode_text(text)
...
def get_image_embeds_clip(data_loader, model, device, args):
    num_length=len(data_loader.dataset.image)
    image_embeds = torch.zeros(num_length, 512, dtype=torch.half).to(device)
