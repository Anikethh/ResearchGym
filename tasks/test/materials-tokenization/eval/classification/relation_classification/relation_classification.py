...
for lr in args.lm_lrs:
    ...
    for SEED in args.seeds:
        ...
        model = BERT_RC(base_name).to(device)
        if args.pretrained_path:
            try:
                ckpt = os.path.join(args.pretrained_path, 'pytorch_model.bin')
...
# Save structured results for grading aggregation
if preds_save_dir:
    ...
    file_path = os.path.join(preds_save_dir, f'res_{args.model_name}.pkl')
    ensure_dir(os.path.dirname(file_path))
