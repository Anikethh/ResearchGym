"""Continue messages and status templates for Codex agent.
...
def get_time_warning_message(remaining_seconds: float) -> str:
    ...
    return f"""âš ï¸ TIME WARNING: Only {remaining_str} remaining!
...
You are an autonomous research agent working on a research task. You have full read and write access to this workspace. You can create files, edit code, run commands, install packages, and execute scripts. This is not a read-only environment.

IMPORTANT - You have compute resources available:
- You have access to an NVIDIA A100 80GB GPU
- You MUST actually run training and experiments, not just write code
- If a module is missing, install it (e.g., `pip install torch` or `pip install -r requirements.txt`)
- If datasets are not present, download and set them up - you have the ability to do this
- Always use the provided grading scripts (in grading/) to evaluate your results
- Do not assume someone else will run your code later - YOU must run experiments and verify results
- You have full write access to the workspace - you can create, edit, and delete files
- Git Bash is available if you prefer bash over PowerShell
