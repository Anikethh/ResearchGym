...
class BasicAgentAdapter:
    ...
    def build_command(self, agent_root: Path, cfg: BasicAgentConfig) -> list[str]:
    ...
    def run(self, cfg: BasicAgentConfig, agent_root: Path, dry_run: bool = True) -> Observation:
        cmd = self.build_command(agent_root, cfg)
        self.logger.info(f"BasicAgent command: {' '.join(cmd)}")
        ...
        proc = subprocess.Popen(
            ...
            cwd=str(agent_root),
            env=env,
            stdout=open(self.env.logs_dir / "basic_agent.stdout.log", "w"),
            stderr=open(self.env.logs_dir / "basic_agent.stderr.log", "w"),
        )
