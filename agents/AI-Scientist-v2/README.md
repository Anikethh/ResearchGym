...
Note: When integrated in ResearchGym, the system can accept a Markdown `task_description.md` directly. It will be wrapped into the minimal JSON schema internally (Title/Abstract/Hypothesis/Experiments/Risks) without requiring a separate conversion step. The ideation stage then runs per task to generate concrete ideas automatically.
