...
def detect_duplicate_figures(client, client_model, pdf_path):
    ...
    try:
        response = _make_tracked_duplicate_detection_call(
            client, client_model, messages
        )
...
@track_token_usage
def _make_tracked_duplicate_detection_call(client, model, messages):
    """Helper function to track tokens for duplicate detection API calls."""
    return client.chat.completions.create(
        model=model,
        messages=messages,
        max_tokens=1000,
        system_message="Duplicate figure detection",
        prompt=f"Analyzing {len(messages)} messages for duplicate detection"
    )
