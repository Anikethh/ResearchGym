...
class AgentManager:
    def __init__(self, task_desc: str, cfg: Any, workspace_dir: Path):
        self.task_desc = task_desc.strip()
        self.cfg = cfg
        ...
        self.main_stage_goals: Dict[int, str] = {
            1: """
                - Build an end-to-end training and evaluation pipeline tailored to the provided research task
                - Operate within the workspace `input` directory and task files; prefer reusing any task-provided baselines or helpers where applicable
                - Aim for functional correctness that can be executed by the task’s grading or run script (if present)
            """,
            2: """
                - Tune hyperparameters, training schedules, or evaluation settings to improve task metrics while keeping data sources consistent with the task setup
                - Maintain architectural continuity unless a task baseline explicitly encourages major changes
                - Report results using the task’s evaluation protocol(s), with clear settings and reproducible commands
            """,
            3: """
                - Explore novel improvements within the task constraints (e.g., better augmentations, training strategies, ensembling)
                - Design experiments that reveal insights about generalization or robustness relevant to the task
                - Keep paths and configuration reproducible via environment variables and/or config files so downstream tooling can reproduce runs
            """,
            4: """
                - Conduct systematic ablation studies on the components you introduced, measuring their impact on the task’s metrics
                - Summarize how each component affects performance and provide rationale for observed trends
            """,
        }
    ...
    def _get_task_desc_str(self) -> str:
        heading = (
            "You are working on an autonomous research benchmark.\n"
            "- Your objective is to improve the official result tables for this task by proposing, implementing, and evaluating new methods.\n"
            "- Follow the instructions below exactly; they supersede any default assumptions.\n\n"
        )
        return heading + self.task_desc + "\n"
    ...
    def _curate_task_desc(self, stage: Stage) -> str:
        return self._get_task_desc_str()
    ...
    def _parse_vlm_feedback(self, node: Node) -> str:
        """Placeholder to maintain compatibility when VLM analysis is disabled."""
        return "No automated plot analysis available for this run.\n"
