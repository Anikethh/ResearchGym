...
class MinimalAgent:
    ...
    def _prompt_impl_guideline(self):
        impl_guideline = [
            "Workspace usage:",
            "  - Read task assets from the read-only `input/` directory prepared for the task (also exported as $RG_TASK_ROOT).",
            "  - Write intermediates and outputs to `working/` (or paths under $RG_WORKING_DIR) without modifying `input/`.",
            "Data and evaluation discipline:",
            "  - Use the datasets, splits, configs, and evaluation commands described in `task_description.md`.",
            "  - Do not fabricate new datasets or metrics; focus on improving the official benchmark tables.",
            "  - Before invoking any task script, run `--help` or read the source to learn real arguments, inputs, and outputs; never invent flags.",
            "  - Datasets are not prepackaged; download/extract into $RG_TASK_ROOT/data using the URLs in task_description.md (PAM/boiler/epilepsy from Dataverse; Wafer/Freezer from UCR archive; synthetic via `python synthetic/switchstate/switchgenerator.py`). Use curl/wget + unzip/tar as needed.",
            "  - Proactively install required Python deps in the current environment before running scripts: at minimum `pip install timesynth` for synthetic generation.",
            "Running experiments:",
            "  - Invoke the task's training/evaluation entry points (e.g., `python run.py`, `bash grading/grade.sh`, or scripts under `grading/`) to generate metrics; run grading via `bash grading/grade.sh` (Git Bash on Windows is available via $RG_BASH_PATH).",
            "  - Prefer `subprocess.run([...], check=True)` or similar orchestration instead of re-implementing full training loops.",
            "  - Keep paths relative and reproducible; rely on environment variables documented by the task for external resources.",
            "Logging results:",
            "  - Capture the official metrics in stdout and, when feasible, parse report files or JSON emitted by the grading script.",
            "  - Structure any saved outputs to match the metric names in the task documentation (e.g., JSON, CSV).",
            "Device handling:",
            "  - Detect the device once using `torch.device('cuda' if torch.cuda.is_available() else 'cpu')` and move models/tensors accordingly.",
            "  - Create optimizers after moving models to the device and move batch tensors inside the training loop.",
            "Code structure:",
            "  - Keep orchestration code concise and focused on reusing existing scripts/configs rather than rebuilding the entire pipeline from memory.",
            "  - Operate directly from $RG_TASK_ROOT (already set to the task copy); do not attempt to copy an extra nested `input/`.",
            "  - Start scripts by creating `working_dir = os.path.join(os.getcwd(), 'working')` and `os.makedirs(working_dir, exist_ok=True)`.",
            "  - Keep execution at module scope so scripts run immediately without requiring `if __name__ == '__main__':` guards.",
            "  - Ensure runs complete within "
            + humanize.naturaldelta(self.cfg.exec.timeout)
            + ".",
        ]
    ...
    @property
    def _prompt_task_adherence(self):
        return {
            "Task adherence": [
                "Review the materials in `input/` (e.g., `task_description.md`, baselines, configs) before proposing changes.",
                "When evaluating, call the task's provided scripts or entry points (`run.sh`, `run.py`, `grading` utilities) so results align with the official grading pipeline.",
                "Do not fabricate new datasets or benchmarks; improvements must target the metrics described in the documentation.",
                "After running the official scripts, parse their outputs (tables, JSON, logs) to populate the benchmark metrics you report.",
                "Keep commands reproducible and note any environment variables or setup steps required by the task. Avoid unsupported datasets (e.g., mimic3) unless explicitly present; prefer the documented real/synthetic datasets.",
            ]
        }
    ...
    def _prompt_resp_fmt(self):
        return {
            "Response format": (
                ...
                "The code should orchestrate the task's existing scripts (e.g., via `subprocess.run`) instead of re-implementing the entire training loop. "
    ...
    def _prompt_metricparse_resp_fmt(self):
        return {
            "Response format": (
                ...
                "There should be no additional headings or text in your response. Just natural language text followed by a newline and then the markdown code block. "
                "Your generated code should be complete and executable, and should parse the outputs emitted by the task's official scripts."
            )
        }
    ...
    def _prompt_debug_resp_fmt(self):
        return {
            "Response format": (
                ...
                "There should be no additional headings or text in your response. Just natural language text followed by a newline and then the markdown code block. "
                "Keep the implementation focused on reusing the task scripts with minimal adjustments; do not rewrite the full training loop. "
                "Make sure to write concise code."
            )
        }
    ...
    def _prompt_hyperparam_tuning_resp_fmt(self):
        return {
            "Response format": (
                ...
                "Drive the tuning by calling the official training/evaluation scripts with adjusted arguments/configuration rather than rebuilding the pipeline. "
                "Make sure to write concise code."
            )
        }
    ...
    def _prompt_ablation_resp_fmt(self):
        return {
            "Response format": (
                ...
                "There should be no additional headings or text in your response. Do not omit any part of the code. "
                "Structure the ablation by invoking the existing task scripts with modified configuration to isolate each component. "
                "Make sure to write concise code."
            )
        }
    ...
    def _draft(self) -> Node:
        ...
        prompt["Instructions"] |= {
            "Experiment design sketch guideline": [
                "This first experiment design should focus on the datasets and scripts provided with the task.",
                "Take the Memory section into consideration when proposing the design.",
                "The solution sketch should be 6-10 sentences.",
                "Don't suggest to do EDA or fabricate new datasets.",
                "",
            ],
            "Evaluation Metric(s)": self.evaluation_metrics,
        }
        prompt["Instructions"] |= self._prompt_task_adherence
    ...
    def _generate_hyperparam_tuning_node(
        ...
        prompt["Instructions"] |= {
            "Implementation guideline": [
                "Keep the solution organized and executable within the task workspace.",
                "Invoke the task's official training/evaluation commands with the necessary hyperparameter adjustments.",
                "Persist intermediate metrics or artifacts only when needed by the task's tooling (reuse existing formats such as JSON or CSV).",
            ]
        }
    ...
    def _generate_ablation_node(self, parent_node: Node, ablation_idea: AblationIdea):
        ...
        prompt["Instructions"] |= {
            "Implementation guideline": [
                "Keep the solution organised and executable for the ablation study.",
                "Execute the task's official training/evaluation commands with ablation-specific modifications.",
                "Record metrics using the same mechanisms as the baseline scripts (stdout tables, JSON artefacts, etc.) only when required.",
            ]
        }
...
class ParallelAgent:
    ...
    def _define_global_metrics(self) -> str:
        ...
        guidance = (
            "- Use the evaluation metrics and result tables specified in the task documentation.\n"
            "- Run the provided training/evaluation scripts (e.g., `run.sh`, `grade.sh`, or task-specific Python entry points) to compute those metrics.\n"
            "- Record the official metrics in your stdout and optionally into `experiment_data.npy` so downstream tooling can summarize them.\n"
            "- Do not introduce new metrics or proxy objectives unless the task instructions explicitly allow it."
        )
        print(f"[green]Defined eval metrics guidance:[/green] {guidance}")
        return guidance
    ...
    def _process_node_wrapper(
        ...
        # Ensure task assets are available inside the process workspace
        workspace_path = Path(workspace)
        shared_input = Path(cfg.workspace_dir) / "input"
        process_input = workspace_path / "input"
        if shared_input.exists():
            if process_input.exists():
                shutil.rmtree(process_input, ignore_errors=True)
            try:
                shutil.copytree(shared_input, process_input)
            except Exception as copy_err:
                logger.warning(
                    "Failed to copy task assets into process workspace %s: %s",
                    process_input,
                    copy_err,
                )
        ...
        # Run interpreter from the task root (input/) so relative paths match task scripts
        interp_cwd = process_input if process_input.exists() else workspace_path
        env_vars = {
            # Give the agent explicit roots to discourage path guessing
            "RG_TASK_ROOT": str(interp_cwd),
            "RG_WORKING_DIR": str(Path(working_dir)),
            # Make repository imports resolvable without manual PYTHONPATH edits
            "PYTHONPATH": str(interp_cwd),
        }
        # Ensure Bash is discoverable for grade.sh on Windows if Git Bash is installed
        bash_candidates = [
            os.environ.get("RG_BASH_PATH"),
            r"C:\Program Files\Git\bin\bash.exe",
            r"C:\Program Files\Git\usr\bin\bash.exe",
        ]
        bash_path = next((b for b in bash_candidates if b and Path(b).exists()), None)
        if bash_path:
            env_vars["RG_BASH_PATH"] = bash_path
            env_vars["PATH"] = f"{Path(bash_path).parent}{os.pathsep}" + os.environ.get(
                "PATH", ""
            )
        process_interpreter = Interpreter(
            working_dir=interp_cwd,
        ...
        ablation_prompt = {
            ...
            "Instructions": {
                "Requirements": [
                    ...
                    "4. If you have only exercised a subset of the official benchmarks, include an ablation that revisits the remaining benchmarks or data splits defined in the task instructions.",
                ]
            },
            "Response format": (
    ...
    def step(self, exec_callback: ExecCallbackType):
        ...
        print("Preparing execution tasks")
        tasks: List[dict[str, Any]] = []
        for index, node_data in enumerate(node_data_list):
            process_id = f"worker_{index}"
            gpu_id = None
            if self.gpu_manager is not None:
                try:
                    gpu_id = self.gpu_manager.acquire_gpu(process_id)
            ...
            tasks.append(
                {
                    ...
                    "best_stage1_plot_code": self.best_stage1_node.plot_code
                    if self.best_stage1_node
                    else None,
                    "best_stage2_plot_code": self.best_stage2_node.plot_code
                    if self.best_stage2_node
                    else None,
                    "best_stage3_plot_code": self.best_stage3_node.plot_code
                    if self.best_stage3_node
                    else None,
                    "seed_eval": False,
                }
            )

        # Execute tasks either synchronously or via process pool
        if self.executor is None:
            print("Running tasks synchronously")
            for task in tasks:
                try:
                    result_data = self._process_node_wrapper(
                        task["node_data"],
                        self.task_desc,
                        self.cfg,
                        task["gpu_id"],
                        ...
                        self.evaluation_metrics,
                        self.stage_name,
                        task["new_ablation_idea"],
                        task["new_hyperparam_idea"],
                        task["best_stage1_plot_code"],
                        task["best_stage2_plot_code"],
                        task["best_stage3_plot_code"],
                        task["seed_eval"],
                    )
                    ...
                    if (
                        ...
                        and task["process_id"] in self.gpu_manager.gpu_assignments
                    ):
                        self.gpu_manager.release_gpu(task["process_id"])
                        logger.info(f"Released GPU for process {task['process_id']}")
        else:
    ...
    def cleanup(self):
        ...
        if not self._is_shutdown:
            ...
            try:
                ...
                if self.executor is not None and getattr(self.executor, "_processes", None):
