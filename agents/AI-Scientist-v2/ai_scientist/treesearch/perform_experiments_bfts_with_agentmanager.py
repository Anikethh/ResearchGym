...
def perform_experiments_bfts(config_path: str):
    ...
    # Save cost information at the end of execution
    try:
        from ai_scientist.utils.cost_tracker import ai_scientist_cost_tracker
        
        # Get final cost summary
        cost_summary = ai_scientist_cost_tracker.get_summary()
        
        # Save cost summary to file
        cost_summary_path = cfg.log_dir / "ai_scientist_cost_summary.json"
        ...
        print(f"💾 Cost details saved to: {cost_summary_path}")
        
        # Log per-model breakdown
        for model, model_data in cost_summary.items():
            if isinstance(model_data, dict) and "cost_usd" in model_data:
                tokens = model_data.get("tokens", {})
                print(f"  📊 {model}: ${model_data['cost_usd']:.6f} "
                      f"({tokens.get('total_tokens', 0)} tokens)")
        
        logger.info(f"AI-Scientist cost summary saved to: {cost_summary_path}")
        
    except Exception as e:
        logger.error(f"Failed to save AI-Scientist cost summary: {e}")
        print(f"⚠️  Failed to save cost summary: {e}")
