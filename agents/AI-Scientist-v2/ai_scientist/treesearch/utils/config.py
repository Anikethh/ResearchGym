...
def prep_cfg(cfg: Config):
    # Allow optional data_dir; when not provided, the agent will work entirely in its workspace
    # and may create or fetch data programmatically during experimentation.
    ...
    if cfg.data_dir and cfg.data_dir.startswith("example_tasks/"):
...
def load_task_desc(cfg: Config) -> str:
    ...
    if cfg.desc_file is not None:
        ...
        return Path(cfg.desc_file).read_text(encoding="utf-8")
    ...
    if cfg.eval is not None:
        parts.append(f"Evaluation guidance:\n{cfg.eval}")
...
def prep_agent_workspace(cfg: Config):
    ...
    # If a data directory is provided, mirror it into the workspace input directory.
    if cfg.data_dir is not None and Path(cfg.data_dir).exists():
