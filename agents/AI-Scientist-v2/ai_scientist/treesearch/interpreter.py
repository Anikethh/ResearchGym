...
class RedirectQueue:
    ...
    # Some subprocess calls expect a fileno-capable stream; provide a real fd from the process stdout.
    def fileno(self):
...
class Interpreter:
    ...
    def child_proc_setup(self, result_outq: Queue) -> None:
        ...
        # Safely set environment variables with None value check
        for key, value in self.env_vars.items():
            try:
                if value is not None:
                    os.environ[key] = str(value)
                # Silently skip None values without logging to avoid spam
            except Exception as e:
                # Only log actual errors, not None values
                if value is not None:
                    import logging
                    logging.getLogger("ai-scientist").warning(f"Failed to set environment variable {key}: {e}")
                ...
                if running_time > self.timeout:
                    ...
                    if not child_in_overtime:
                        try:
                            ...
                            child_in_overtime = True
                            logger.warning(
                                "Failed to deliver SIGINT to child process (pid=%s): %s. "
                                "Attempting graceful terminate() instead.",
                                self.process.pid,
                                perm_err,
                            )
                            try:
                                ...
                                logger.error(
                                    "terminate() also failed for child process (pid=%s): %s",
                                    self.process.pid,
                                    terminate_err,
                                )
                            ...
                            child_in_overtime = True
                            logger.warning(
                                "Unexpected error sending SIGINT to child process (pid=%s): %s",
                                self.process.pid,
                                sig_err,
                            )
